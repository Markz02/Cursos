...
- `<h1>` a `<h6>`: Define títulos, onde `<h1>` é o mais importante e `<h6>` o menos importante.
- `<b>`: Deixa o texto em negrito.
- `<i>`: Deixa o texto em itálico.
- `<u>`: Sublinha o texto.
- `<strong>`: Deixa o texto em negrito, com ênfase semântica.
- `<em>`: Deixa o texto em itálico, com ênfase semântica.
- `<br>`: Insere uma quebra de linha.
- `<blockquote>`: Define uma citação em bloco.
- `<pre>`: Define texto pré-formatado, preservando espaços e quebras de linha.
- `<span>`: Define uma seção de texto inline, usada para aplicar estilos específicos.
- `<code>`: Define um bloco de código.

### Dicas Importantes ao Adicionar Texto ao HTML

1. **Semântica**: Use tags semânticas (`<strong>`, `<em>`, `<blockquote>`) em vez de tags puramente visuais (`<b>`, `<i>`) sempre que possível. Isso melhora a acessibilidade e SEO.

2. **Acessibilidade**: Adicione atributos `alt` a imagens e use `aria-label` para descrever elementos que precisam de mais contexto. Isso ajuda pessoas com deficiências visuais a entenderem melhor o conteúdo.

3. **Quebras de Linha**: Use `<br>` apenas quando uma quebra de linha lógica é necessária dentro de um parágrafo. Para novos parágrafos, use `<p>`.

4. **Estrutura Hierárquica**: Mantenha uma hierarquia clara usando títulos (`<h1>` a `<h6>`) para organizar o conteúdo. Evite pular níveis de títulos, como ir diretamente de `<h1>` para `<h3>`.

5. **Consistência de Estilo**: Use CSS para estilizar seu texto em vez de adicionar estilos inline diretamente nas tags HTML. Isso mantém o código mais limpo e fácil de manter.
